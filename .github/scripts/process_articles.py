...
# ─── VERSION HEADER ────────────────────────────────────────────────────────────
# v2.1 — 2026-06-19-2019 — ARTICLE_BASE_URL switched to the custom subdomain
#                          (contrarian.foodgamemedia.com.au) so cards/json link to
#                          our own domain instead of github.io. Requires the CNAME
#                          file + DNS record + Pages custom-domain setting.
# v2.0 — 2026-06-19-2019 — strip the engine's 'Tune the closer' editor panel
#                          (#fftControls) from published articles (its Rewrite
#                          buttons are dead on the static site); article base URL
#                          moved to a single ARTICLE_BASE_URL constant for easy
#                          custom-domain switch. Supersedes the standalone v1.9.
# v1.9 — 2026-06-19-2019 — card headline now read from the engine's Playfair-italic
#                          header div with no length cap; removed the {10,120} cap
#                          that silently failed on long headlines and fell back to
#                          the filename slug (mangled "Australia S ... Doesn T ... Source")
...
# ─── STRIP ENGINE EDITOR CHROME ───────────────────────────────────────────────

def strip_editor_controls(html):
    """Remove the engine's 'Tune the closer' panel (#fftControls) from a saved
    article. That panel is app editor UI: tone chips plus a Rewrite button whose
    onclick calls rewriteCloser(), a function that only exists inside the engine,
    so on the published static page the buttons are dead and look broken. The
    Food for Thought block (#fftBlock) is real article content and is kept.

    Index-based balanced-<div> removal, no fragile regex: find the #fftControls
    div, walk div nesting to its matching close, and cut exactly that span.
    Idempotent and safe to re-run."""
    marker = 'id="fftControls"'
    ...
    while j < n:
        ...
        nc = html.find('</div>', j)
        if nc == -1:
            return html  # malformed markup — leave the file untouched
        if nd != -1 and nd < nc:
            depth += 1
            j = nd + 4
        else:
...
# Public base URL for saved articles. Change this ONE line to move articles off
# github.io onto a custom domain (e.g. a foodgamemedia.com.au subdomain). It is
# the only place the article URL is built, so the cards/json follow automatically.
ARTICLE_BASE_URL = 'https://contrarian.foodgamemedia.com.au/articles/'
...
def extract_metadata(filepath):
    ...
    # Pull the real headline from the rendered piece header so the card matches
    # exactly what the engine produced. The engine renders the headline as a
    # Playfair Display, italic <div> in the dark header band. Match that div
    # specifically and do NOT cap its length: real headlines run well past 120
    # chars, and apostrophes are stored as &#039; entities (so "Australia's
    # ... Doesn't ..." is 130 raw chars). The old {10,120} cap silently missed
    # those and fell back to the filename slug, which .title() mangled into
    # "Australia S ... Doesn T ... Source". Diagnosed 19 Jun 2026.
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            doc = f.read()
        import html as _html

        # Primary: the engine's headline div (Playfair Display + italic), any length
        title_match = re.search(
            r"font-family:'Playfair Display'[^>]*font-style:italic[^>]*>(.*?)</div>",
            doc, re.IGNORECASE | re.DOTALL
        )
        if not title_match:
            # Fallback 1: any italic header div (older / externally sourced pieces)
            title_match = re.search(r'font-style:italic[^>]*>([^<]{10,300})</div>', doc, re.IGNORECASE)
        if not title_match:
            # Fallback 2: <title> tag
            title_match = re.search(r'<title>([^<]{5,200})</title>', doc, re.IGNORECASE)

        if title_match:
            candidate = re.sub(r'<[^>]+>', '', title_match.group(1))   # strip any inner tags
            candidate = _html.unescape(candidate).strip()
            # Ignore generic placeholder titles
            if candidate and ('Contrarian' not in candidate or len(candidate) > 20):
                headline = candidate
    except Exception:
        pass
...
def main():
    ...
    for filepath in files:
        ...
        # Remove the engine's 'Tune the closer' editor panel — not for publication
        html = strip_editor_controls(html)

        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(html)
        print(f'  Nav injected, editor controls stripped')
