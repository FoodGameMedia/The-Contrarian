...
# ─── VERSION HEADER ────────────────────────────────────────────────────────────
# v1.3 — 2026-02-28 — replace inline body style so page background is navy, not #f5f5f5
...
.fgm-mobile-menu a {
  ...
  border-bottom: 1px solid rgba(244,167,185,0.22);
}
...
def inject_nav(html):
    # Replace inline body background/margin styles with navy site background
    html = re.sub(
        r'(<body\b[^>]*?)style="[^"]*"',
        r'\1style="margin:0;padding:0;background:#0f1f38;"',
        html, count=1, flags=re.IGNORECASE
    )
    # Replace article body box (light purple → navy)
    html = html.replace('background:#cdc8d9', 'background:#0f1f38')
    html = html.replace('background: #cdc8d9', 'background:#0f1f38')
    # Replace author box (blush pink → dark card)
    html = html.replace('background:#fce4ec', 'background:rgba(255,255,255,0.04);border:1px solid rgba(244,167,185,0.22)')
    html = html.replace('background: #fce4ec', 'background:rgba(255,255,255,0.04);border:1px solid rgba(244,167,185,0.22)')
    # Fix dark text → light for navy background
    html = html.replace('color:#333', 'color:rgba(255,255,255,0.82)')
    html = html.replace('color: #333', 'color:rgba(255,255,255,0.82)')
    html = html.replace('color:#444', 'color:rgba(255,255,255,0.7)')
    html = html.replace('color: #444', 'color:rgba(255,255,255,0.7)')
    html = html.replace('color:#1a1a2e', 'color:rgba(255,255,255,0.82)')
    html = html.replace('color:#0f1f38;font-weight:700', 'color:#ffffff;font-weight:700')
