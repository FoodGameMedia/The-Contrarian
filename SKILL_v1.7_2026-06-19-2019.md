...
**Last updated:** 19 June 2026 (v1.7 — custom subdomain hosting, headline + editor-chrome fixes, publish-pipeline hardening)
...
- **Public host: `contrarian.foodgamemedia.com.au/articles/[filename]`** (custom subdomain on the repo's GitHub Pages, live 19 Jun 2026 — replaces the old `foodgamemedia.github.io/The-Contrarian/...` path so cards open on our own domain). The github.io URL still works (GitHub 301-redirects it to the custom domain), but the subdomain is canonical.
- Custom-domain wiring: a `CNAME` file in the repo root containing exactly `contrarian.foodgamemedia.com.au`; a DNS `CNAME` record `contrarian` → `foodgamemedia.github.io`; and the same domain entered under repo Settings → Pages → Custom domain (Enforce HTTPS on). The auto-commit step never touches `CNAME`, so it persists.
- The public base lives in ONE place in `process_articles.py`: the `ARTICLE_BASE_URL` constant. To move domains again, change that line, the `CNAME` file, and the DNS record.
...
- **Main-site link-up:** `contrarian.html` on `foodgamemedia.com.au` fetches `archive.json`. After the subdomain went live this fetch should point at `https://contrarian.foodgamemedia.com.au/archive.json` (the old github.io path still resolves via redirect but is not canonical).
...
As of `process-articles.yml` **v1.3** (19 Jun 2026) the workflow serialises runs via a top-level `concurrency` group and pushes rebase-safe (fetch + rebase onto remote + retry up to 5×), so two saves in quick succession can no longer collide and strand an article with a rejected push. See gotcha in section 13.

### Files
| File | Location | Purpose |
|------|----------|---------|
| `process-articles.yml` | `.github/workflows/` | GitHub Action — fires on push to `articles/`. v1.3+: `concurrency` group + rebase-safe push. Edit on GitHub directly, NOT via the Worker (needs `workflow` token scope) |
...
[
  {
    ...
    "url": "https://contrarian.foodgamemedia.com.au/articles/DD.MM.YY-slug.html"
  }
]
```
Sorted newest first.
...
### Archive pipeline script (`.github/scripts/process_articles.py`)
| Item | Purpose |
|------|---------|
| `ARTICLE_BASE_URL` (constant) | The single source of the public article URL base. Set to `https://contrarian.foodgamemedia.com.au/articles/` (v2.1). Change this one line + `CNAME` + DNS to move domains |
| `inject_nav()` / `strip_nav()` | Strip old nav and inject the current FGM nav verbatim from the constants at the top of the script |
| `strip_editor_controls()` | Removes the engine's `#fftControls` "Tune the closer" editor panel from each article (its Rewrite buttons are dead on the static site). Keeps `#fftBlock` (real content). Balanced-`<div>` removal by index, no regex. Added v2.0 |
| `extract_metadata()` | Date from filename; headline read from the engine's Playfair-italic header div (no length cap, v1.9). Falls back to filename slug only for old externally-sourced imports with no engine header |
...
- **Article saves but never publishes (Action fails at push).** Saving an article to `articles/` and it appearing on the site are two separate steps. The save (engine push to `articles/`) can succeed while the post-save Action (`process-articles.yml` → `process_articles.py`) fails, leaving the file in the repo with no nav injected and absent from `archive.json`/`archive.html`, so no card shows on `contrarian.html`. The Python script is rarely the cause (it is a full rebuild and runs clean on its own) — the `process` job exiting 1 is generic ("Process completed with exit code 1"), so expand the red step to see whether the Python step or the push step died; read the step log, not just the status. First diagnosed 19 Jun 2026 on the Australia hospitality piece (run #27): a `git push` race. With no concurrency guard, two overlapping runs collided and the loser's push was rejected non-fast-forward, exiting 1. **Immediate recovery:** open the failed run in the Actions tab and hit **Re-run jobs** — it re-checks-out, re-runs the script, and republishes within ~5 min. **Permanent fix:** `process-articles.yml` v1.3 (19 Jun 2026) — a `concurrency` group serialises runs and the commit step fetches + rebases onto the latest remote and retries the push up to 5×.
- **Workflow files cannot be deployed through the app/Worker.** `index.html`, articles, and `SKILL.md` push fine via the Worker `/github` route (`GITHUB_TOKEN` is a classic PAT, repo scope). Files under `.github/workflows/` are special: GitHub rejects a classic PAT lacking the `workflow` scope from creating or updating any workflow file (403). So `process-articles.yml` must be edited directly on GitHub (web editor: open the file → pencil → Commit) or via local git with a suitably-scoped token — never through the engine. (Note: `.github/scripts/process_articles.py` is NOT a workflow file, so repo scope can write it — but in practice edit it on GitHub too, for consistency.)
- **Card headline must come from the rendered header, not the filename.** `process_articles.py` reads the card/JSON headline from the engine's Playfair Display + italic header `<div>`. The old extractor capped the match at `{10,120}` chars; real headlines run longer (and apostrophes are stored as `&#039;`, ~6 chars each), so long headlines silently missed and the script fell back to the filename slug, which `.title()` mangled into things like "Australia **S** ... Doesn **T** ... Five **Source**". Fixed v1.9: match the header div specifically, no length cap, unescape entities. The filename slug is now a last resort only for old externally-sourced imports that have no engine header (e.g. "Fin Review", the ABC piece). Symptom to watch for: a card title that is Title-Cased with stray capital letters after apostrophes and a trailing source word — that means extraction missed and it fell back to the slug.
- **The engine bakes editor chrome into saved articles.** `renderPieceHTML()` includes the `#fftControls` "Tune the closer" panel (tone chips + Rewrite button calling `rewriteCloser()`) in the saved HTML. On the published static page those buttons are dead and look broken. The pipeline strips `#fftControls` on every run (`strip_editor_controls()`, v2.0), so published articles are clean regardless of what the engine emits — but the raw saved file still contains the panel until processed. The proper source fix (have `renderPieceHTML` not emit the panel into the saved string) is still open; the pipeline strip fully covers the live-site symptom in the meantime.
